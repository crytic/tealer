...
def _is_oncompletion_check(ins1: Instruction, ins2: Instruction):
    if isinstance(ins1, Txn) and isinstance(ins1.field, OnCompletion):
        return isinstance(ins2, Int) and ins2.value in ["DeleteApplication", "NoOp", "OptIn", "CloseOut"]
    return False


class CanUpdate(AbstractDetector):
    NAME = "canUpdate"
    ...
    def detect(self):
        ...
        for path in paths_without_check:
            ...
            idx += 1
            description = f"Lack of OnCompletion check allows to update the app\n"
